...
jQuery.extend({
    navFn:function (data) {
        ...
        for(var i = 0;i<data.length; i++){
            m += '<div class="dl" id="nav-id-'+data[i].id+'">';
            m += '  <a class="dt" href="' + data[i].url+'">';
            m += '      <div class="l"><i class="iconfont">'+data[i].iconfont+'</i></div>';
            m += '      <div class="r"><i class="iconfont">&#xe60b;</i></div>';
            m += '      <div class="m">';
            m += '          <span>'+data[i].name+'</span>';
            m += '      </div>';
            m += '      <div class="clearBoth"></div>';
            m += '  </a>';
            m += '  <div class="dh">';
            m += '      <div class="title">'+data[i].name+'</div>';
            for(var j = 0;j<data[i].sub.length; j++){
                m += '  <a class="dd" href="'+ base._url + data[i].sub[j].url+'" id="nav-id-'+data[i].id+'-'+data[i].sub[j].id+'">';
                m += '      <div class="l"><div class="ic"></div></div>';
                m += '      <div class="m">';
                m += '          <span>'+data[i].sub[j].name+'</span>';
                m += '      </div>';
                m += '      <div class="clearBoth"></div>';
                m += '  </a>';
            }
        ...
        $(".nav-main").html(m);
        var nav_cookie = $.cookie('nav_cookie2');
        //获取cookie 设定导航状态
        if(nav_cookie=="on"){
            $(".b-nav-fn").removeClass("in");
            $(".b-nav").stop().addClass("on");
            $.addNavIn("set");
        }
        else{
            $.addNavIn("set");
            $.addNavIn("clear");
            $(".b-nav").stop().addClass("off");
            $(".b-nav-fn").addClass("in");
        }
        //动态设置导航触发事件是会发生动画效果
        setTimeout(function(){
            $(".b-nav").addClass("animate");
        },500);

        console.log(nav_cookie);
        //导航二级展开关闭
        $(".nav-main .dt").click(function(){
            var _this = $(this).parent(".dl");
            if(!_this.hasClass("in")){
                _this.stop().addClass("in");
                _this.siblings().stop().removeClass("in");
            }
            else{
                _this.stop().removeClass("in");
            }
        });
        //导航展开收缩
        $(".b-nav-fn").click(function(){
            if(!$(this).hasClass("in")){
                $(this).stop().addClass("in");
                $(this).siblings().stop().removeClass("in");
                $(".b-nav").stop().addClass("off");
                $(".b-nav").stop().removeClass("on");
                $.cookie('nav_cookie2','off',{ expires: 7, path:"/"});
                console.log("off");
                console.log($.cookie('nav_cookie2'));
                $.addNavIn("clear");
            }
            else{
                $(this).stop().removeClass("in");
                $(".b-nav").stop().addClass("on");
                $(".b-nav").stop().removeClass("off");
                $.cookie('nav_cookie2','on',{ expires: 7, path:"/"});
                console.log("on");
                $.addNavIn("set");
            }
        });
...
//设置导航选中状态， set 设置 clear 清楚
jQuery.extend({
    addNavIn:function (state) {
        if(state=="set"){
            $("#nav-id-"+$("#navVal1").val()).addClass("on");
            $("#nav-id-"+$("#navVal1").val()+'-'+$("#navVal2").val()).addClass("on");
        }
        else if(state=="clear"){
            //$(".b-nav .dl").removeClass("on");
            //$(".b-nav .dd").removeClass("on");
            $(".b-nav .dl").removeClass("in");
        }
    }
});

function b_form_error(msg){
    return '<div class="b-title margin r5 b-title-red"><p>'+msg+'</p></div>';
}
...
/* 分页 */
jQuery.extend({
    pageLwFn:function (name,pageNow,pageRow,meiye,heji) {
        var style = "";
        var m = "";
        var listpageNow = parseInt(pageNow);
        var listpageRow = parseInt(pageRow);
        
        m += "<div class='pageLw-style pageLw-style-"+style+"'>"+"<span style='margin-right:20px;'>合计："+heji+"条</span><span style='margin-right:20px;'>每页"+meiye+"条</span>";
        if(listpageNow>3){
            //m +='<div class="l">'
            m += "<a href=\"javascript:"+name+"(1)\">1</a>";
            if(listpageNow>4){
                m += "<a href=\"javascript:"+name+"(2)\">2</a>";
                if(listpageNow>5){
                    m += '<span class="s1">……</span>';
                }
            }
        }
        if((listpageNow-2)>0){m += "<a href=\"javascript:"+name+"("+(listpageNow-2)+")\">"+(listpageNow-2)+"</a>";}
        if((listpageNow-1)>0){m += "<a href=\"javascript:"+name+"("+(listpageNow-1)+")\">"+(listpageNow-1)+"</a>";}

        m += '<span class="s2">'+(listpageNow)+'</span>';


        if((listpageNow+1)<=listpageRow){m += "<a href=\"javascript:"+name+"("+(listpageNow+1)+")\">"+(listpageNow+1)+"</a>";}
        if((listpageNow+2)<=listpageRow){m += "<a href=\"javascript:"+name+"("+(listpageNow+2)+")\">"+(listpageNow+2)+"</a>";}

        if(listpageRow>(listpageNow+4)){
            m += '<span class="s1">……</span>';
        }
        if(listpageRow>(listpageNow+3)){
            m += "<a href=\"javascript:"+name+"("+(listpageRow-1)+")\">"+(listpageRow-1)+"</a>";
        }
        if(listpageRow>(listpageNow+2)){
            m += "<a href=\"javascript:"+name+"("+listpageRow+")\">"+listpageRow+"</a>";
        }
